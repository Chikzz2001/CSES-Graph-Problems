...
void dfs(vector<vector<int>>& graph,int node,vector<int>& vis,vector<int>& order,int id)
{
    vis[node]=id;
...
void solve()
{
    ...
    for(int i=n-1;i>=0;i--)
    {
        ...
        {
            SCC++;
            dfs(rev_graph,order[i],res,t,SCC);
        }
    }
    cout<<SCC<<"\n";
